...
SRC_CRT="${BUNDLE_DIR}/level3-server/level3-server.crt.pem"
SRC_KEY="${BUNDLE_DIR}/level3-server/level3-server.key.pem"
...
DEST_CRT="level3-server/certs/level3-server.crt.pem"
DEST_KEY="level3-server/private/level3-server.key.pem"

[[ -f "$SRC_KEY" && -f "$SRC_CRT" ]] || { echo "Missing level3 server bundle ($SRC_KEY, $SRC_CRT)"; exit 1; }

mkdir -p root/certs level1/certs level2/certs level3-client/certs level3-server/private level3-server/certs

[[ -e "$DEST_KEY" || -e "$DEST_CRT" ]] && { echo "Refusing to overwrite existing level3 server key/cert ($DEST_KEY, $DEST_CRT)"; exit 1; }
...
echo "Level 3 Server import complete:"
echo "  Key : $DEST_KEY"
echo "  Cert: $DEST_CRT"
